...
from app.config import CRON_JOB_DAY, PDB_FTP_STATUS_URL
...
def process_valid(new: bool):
    ...
    with db_context() as session:
        ...
        for id in added:
            ...
            if error:
                failed.append(
                    (full_id, f"Received error code {error} during file fetch")
                )
            else:
                ...
                if not result:
                    failed.append(
                        (full_id, f"Failure during insertion of new version. {result}")
                    )
