...
class StatisticsService:
    ...
    def get_channel_counts_by_id(self, structure_id: int) -> dict:
        result = self.channel_repository.get_channel_counts_by_id(
    ...
    def get_top_5_residues_by_channel(self) -> list[str]:
        "Returns top 5 most common residues in channels."
        result = self.residue_repository.get_top_residue_counts_by_channels()

        return result

    def get_top_5_categories_as_ratios(self) -> dict:
        """Returns top 5 categories of channels as a ratio"""
        result = self.channel_repository.get_top_channel_counts_per_category_ratio()
