...
def validate_uniprot_id(uniprot_id: str):
    ...
    if req.status_code > 500:
        raise HTTPException(
            status_code=503,
            detail=f"UniProt API returned an error when accessing: {req.url}",
        )
    if req.status_code != 200:
        raise HTTPException(
            status_code=404, detail=f"Cannot find Uniprot ID '{uniprot_id}'"
        )
    return uniprot_id
...
uniprot_id_404_response = {
    404: {"description": "Uniprot ID not found", "model": IDError}
}
...
PDB_ID_Type = Annotated[
    str,
    AnnotationPath(description="PDB ID", pattern=PDB_ID_REGEX),
    AfterValidator(validate_pdb_id),
]
Uniprot_ID_Type = Annotated[
    str,
    AnnotationPath(description="Uniprot ID", pattern=UNIPROT_ID_REGEX),
    AfterValidator(validate_uniprot_id),
]
