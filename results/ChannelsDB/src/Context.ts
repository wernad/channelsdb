...
export class Context {
    ...
    async loadChannelData(url: string, pid: string) {
        const response = await fetch(`${url}/channels/${pid}`);
        if (!response.ok) {
            this.apiError = true;
    ...
    async loadAnnotations(url: string, pid: string) {
        const response = await fetch(`${url}/annotations/${pid}`)
        if (!response.ok) {
            this.apiError = true;
            return { Error: await response.json(), apiStatus: response.status };
        }
