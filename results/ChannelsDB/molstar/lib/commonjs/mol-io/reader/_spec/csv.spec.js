...
const parser_1 = require("../csv/parser");
...
describe('csv reader', () => {
    it('basic', async () => {
        const parsed = await (0, parser_1.parseCsv)(csvStringBasic).run();
        if (parsed.isError)
            return;
    ...
    it('advanced', async () => {
        const parsed = await (0, parser_1.parseCsv)(csvStringAdvanced).run();
        if (parsed.isError)
            return;
        const csvFile = parsed.result;
        const strCol = csvFile.table.getColumn('StrCol');
    ...
    it('tabs', async () => {
        const parsed = await (0, parser_1.parseCsv)(tabString, { delimiter: '\t' }).run();
        if (parsed.isError)
            return;
        const csvFile = parsed.result;
        const strCol = csvFile.table.getColumn('StrCol');
