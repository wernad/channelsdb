...
const parser_1 = require("../gro/parser");
...
describe('gro reader', () => {
    it('basic', async () => {
        const parsed = await (0, parser_1.parseGRO)(groString).run();
        if (parsed.isError) {
            console.log(parsed);
            return;
        }
    ...
    it('high precision', async () => {
        const parsed = await (0, parser_1.parseGRO)(groStringHighPrecision).run();
        if (parsed.isError) {
            console.log(parsed);
            return;
        }
