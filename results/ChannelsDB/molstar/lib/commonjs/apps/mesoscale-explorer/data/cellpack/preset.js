...
async function createCellpackHierarchy(plugin, trajectory) {
    ...
    const compRoot = await state.build()
        .toRoot()
        .applyOrUpdateTagged('group:comp:', state_1.MesoscaleGroup, { ...groupParams, root: true, index: -1, tag: `comp:`, label: 'compartment', color: { type: 'custom', value: names_1.ColorNames.white, variability: 20, shift: 0, lightness: 0, alpha: 1 } }, { tags: 'group:comp:', state: { isCollapsed: false, isHidden: groupParams.hidden } })
        .commit();
    const funcRoot = await state.build()
        .toRoot()
        .applyOrUpdateTagged('group:func:', state_1.MesoscaleGroup, { ...groupParams, root: true, index: -1, tag: `func:`, label: 'function', color: { type: 'custom', value: names_1.ColorNames.white, variability: 20, shift: 0, lightness: 0, alpha: 1 } }, { tags: 'group:func:', state: { isCollapsed: false, isHidden: groupParams.hidden } })
        .commit();
    if (entities._rowCount > 1) {
        ...
        for (let i = 0; i < entities._rowCount; i++) {
            ...
            for (let j = 0, jl = nodes.length - 1; j < jl; ++j) {
                ...
                if (!compGroups.has(n)) {
                    ...
                    const group = await state.build()
                        .to(parent)
                        .applyOrUpdateTagged(`group:comp:${n}`, state_1.MesoscaleGroup, { ...groupParams, root: parent === compRoot, index: colorIdx, tag: `comp:${n}`, label, color: { type: 'generate', value: color, variability: 20, shift: 0, lightness: 0, alpha: 1 } }, { tags: `comp:${p}`, state: { isCollapsed: true, isHidden: groupParams.hidden } })
            ...
            if (!funcGroups.has(f)) {
                ...
                const group = await state.build()
                    .to(funcRoot)
                    .applyOrUpdateTagged(`group:func:${f}`, state_1.MesoscaleGroup, { ...groupParams, index: colorIdx, tag: `func:${f}`, label: f, color: { type: 'custom', value: color, variability: 20, shift: 0, lightness: 0, alpha: 1 } }, { tags: 'func:', state: { isCollapsed: true, isHidden: groupParams.hidden } })
                    .commit({ revertOnError: true });
