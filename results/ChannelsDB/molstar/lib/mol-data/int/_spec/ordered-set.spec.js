...
describe('ordered set', () => {
    function ordSetToArray(set) {
        const ret = [];
        for (let i = 0, _i = OrderedSet.size(set); i < _i; i++)
            ret.push(OrderedSet.getAt(set, i));
        return ret;
    }
    ...
    const iB = (s, e) => Interval.ofBounds(s, e);
    testEq('empty', empty, []);
    ...
    it('isSubsetIS', () => {
        expect(OrderedSet.isSubset(Interval.ofRange(1271, 1295), OrderedSet.ofSortedArray([1271, 1272, 1274, 1275, 1276, 1278, 1280, 1282, 1284, 1286, 1288, 1290, 1292, 1294]))).toBe(true);
    });
    ...
    testEq('idxIntersect 1', OrderedSet.indexedIntersect(OrderedSet.ofSortedArray([1, 2, 4]), SortedArray.ofSortedArray([1, 2, 3, 4, 5, 6]), SortedArray.ofSortedArray([2, 4, 5, 8])), [0, 2]);
    testEq('idxIntersect 2', OrderedSet.indexedIntersect(OrderedSet.ofSortedArray([0, 1]), SortedArray.ofSortedArray([1, 2]), SortedArray.ofSortedArray([1, 2])), [0, 1]);
    ...
    it('foreach', () => {
        ...
        expect(OrderedSet.forEach(int, (v, i, ctx) => ctx[i] = v, [])).toEqual([1, 2]);
        expect(OrderedSet.forEach(set, (v, i, ctx) => ctx[i] = v, [])).toEqual([2, 3, 4]);
    });
});
